...
from src.bot.keyboards.clickup import menu
from src.bot.messages.clickup.tasks import (
    prepare_response_list_tasks, prepare_response_list_tasks_with_unset_time
)


@dp.message_handler(Text(equals=[menu.MenuClickUpKeysEnum.task_list.value]), state=None)
async def get_my_tasks(message: types.Message):
    ...
    await message.answer(f"{response}", parse_mode=ParseMode.HTML, reply_markup=menu.keyboards)


@dp.message_handler(Text(equals=[menu.MenuClickUpKeysEnum.task_list_with_unset_time.value]), state=None)
